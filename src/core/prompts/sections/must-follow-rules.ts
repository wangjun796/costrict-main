...
export function getMustFollowRules(_shell?: string): string[] {
	...
	// Shell rule - single consolidated pointer; full detail lives in SYSTEM INFORMATION
	// (see getSystemInfoSection) and the RULES section. Avoids triplicated shell guidance.
	rules.push(
		`- **RULE: All commands MUST be compatible with the Current Shell (see SYSTEM INFORMATION) — syntax, chaining, quoting, and utilities. Execution MUST use UTF-8.**`,
	)
...
export function getLiteMustFollowRules(_shell?: string, _settings?: SystemPromptSettings): string[] {
	...
	// Shell rule - single consolidated pointer (simplified)
	rules.push(
		`- Commands must be compatible with the Current Shell (see SYSTEM INFORMATION); UTF-8 only`,
	)
